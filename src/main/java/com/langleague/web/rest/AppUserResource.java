...
/**
 * REST controller for managing {@link com.langleague.domain.AppUser}.
 */
@RestController
...
public class AppUserResource {
    ...
    @GetMapping("/me")
    public ResponseEntity<AppUserDTO> getCurrentUserAppUser() {
        LOG.debug("REST request to get current user's AppUser");
        String login = SecurityUtils.getCurrentUserLogin()
            .orElseThrow(() -> new BadRequestAlertException("User not authenticated", ENTITY_NAME, "unauthenticated"));
        Optional<AppUserDTO> appUserDTO = appUserService.findByUserLogin(login);
        return ResponseUtil.wrapOrNotFound(appUserDTO);
    }
